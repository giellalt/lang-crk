...
hfst-lexc $GTLANG_crk/src/derivation/crk-drv-weighted.lexc -o crk-drv-morph-w.hfst
...
hfst-compose -F -x ON -1 crk-drv-morph-w.hfst -2 crk-drv-VG3-filter.hfst -o - |
hfst-compose -F -x ON -1 - -2 crk-drv-phon.hfst -o - |
hfst-compose -x ON -1 - -2 crk-morphophon.hfst -o crk-drv-w.hfst

# hfst-compose -F -x ON -1 crk-drv-morph-w.hfst -2 crk-drv-phon.hfst -o - |
# hfst-compose -x ON -1 - -2 crk-morphophon.hfst -o crk-drv-w.hfst
...
hfst-substitute -i crk-infl-morph.hfst -o crk-drv-infl-morph-w.hfst -f 'DRV-FST' -T crk-drv-w.hfst
echo 'CRK-DRV: Substituted DRV-FST with derivational model.' ;

hfst-xfst -e "load crk-drv-infl-morph-w.hfst" -e "eliminate flag infl" -e "eliminate flag morf" -E "save stack crk-drv-infl-morph-w.hfst"
...
hfst-compose-intersect crk-drv-infl-morph-w.hfst crk-phon.hfst |
hfst-minimize -i - -o crk-gen-norm-w.hfst

echo 'CRT-DRV: Inverting generating FST to create analyzing FST.' ;

hfst-invert -i crk-gen-norm-w.hfst -o crk-anl-norm-w.hfst

echo 'CRK-DRV: Creating optimized versions of both FSTs.' ;

hfst-fst2fst -w -i crk-gen-norm-w.hfst -o crk-gen-norm-w.hfstol
hfst-fst2fst -w -i crk-anl-norm-w.hfst -o crk-anl-norm-w.hfstol
