...
grep ";" $GTHOME/langs/crk/src/morphology/stems/nouns.lexc |egrep -v '^(<|\!|@|AN|IN|IR)' | grep ' IND' |tr ':+' ' ' | cut -d ' ' -f1 | grep -v "^$" |sed 's/$/+N+IN+Sg/' |$HLOOKUP $GTHOME/langs/crk/src/generator-gt-norm.hfst | cut -f2 | grep -v "^$" |sort -u > $GTHOME/langs/crk/test/anlemmas.txt
...
grep ";" $GTHOME/langs/crk/src/morphology/stems/nouns.lexc | egrep -v '^(<|\!|@|AN|IN|IR)' | egrep INALIENABLEPOSS |tr ':+' ' ' | cut -d ' ' -f1 | grep -v "^$" |sed 's/$/+N+IN+Sg+Px3iSg+Obv/' |$HLOOKUP $GTHOME/langs/crk/src/generator-gt-norm.hfst | cut -f2 | grep -v "^$" |sort -u >> $GTHOME/langs/crk/test/anlemmas.txt
